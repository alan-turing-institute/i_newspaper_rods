...
This is a patch to query New Zealand Paper Past:
The archive structure is non-existent, so we cannot go through
all the issues, we go by xml file (which may hold more than one issue, 
or an incomplete issue (with the rest of the ssue in othe xml)
"""
...
class Issue(object):
    ...
    def __init__(self, stream):
        ...
        try:
            ...
            #self.page_count = 0
            #self.day_of_week = ''
 	    print 'Skipping invalid XML from file {}'.format(object)
            #raise error
        ...
        try:
            self.issue = self.single_query('.//search')
        except IndexError:
            self.issue = self.single_query('/search')
